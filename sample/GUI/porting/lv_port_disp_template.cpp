...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
	...
	for (y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
		uint8_t* ptr_dst = buffer + area->x1*(screen_height)*2 + ((screen_height)-1-y)*2; 
		...
		for (int x = 0; x < w; x++) {
			...
			ptr_dst[1] = rgb1555 >> 8;
			ptr_dst += ((screen_height)<<1);   
			color_p++;
		}
	}
	#else //转90
for (y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
		uint8_t* ptr_dst = buffer +  (disp_drv->hor_res-1-area->x1)*disp_drv->ver_res*2 + y*2;
		uint8_t A, Red, GREEN, BLUE = 0;
		uint16_t rgb1555 = 0;
		for (int x = 0; x < w; x++) {
			// alpha=0表示该像素非图标，则A赋值1；否则像素为图标，则A赋值0
