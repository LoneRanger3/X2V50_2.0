...
void PageSysSet::OpenTimeSetPage()
{
	...
	int roller_width = size_w(50), roller_space = size_w(12);
    ...
    lv_style_set_text_font(&roller_style, &lv_font_montserrat_18); 
	...
	lv_style_set_text_font(&roller_selected_style, &lv_font_montserrat_18);
	...
	lv_obj_align(year_roller_, LV_ALIGN_TOP_LEFT, roller_space, size_h(5));
	...
	//btn
	yes_btn_ = lv_create_btn(time_page_, 110, 40, lv_color_make(95, 93, 93), 0, ChangeSysTimeEvent, LV_EVENT_ALL,
		(void*)Btn_Yes, 0);
	...
	lv_obj_align(yes_btn_, LV_ALIGN_BOTTOM_MID, -size_w(80), -size_h(10));

	lv_obj_t* btn_label = lv_label_create(yes_btn_);
	lv_label_set_text(btn_label, GetParsedString("Confirm"));
	lv_obj_center(btn_label);

	no_btn_ = lv_create_btn(time_page_, 110, 40, lv_color_make(95, 93, 93), 0, ChangeSysTimeEvent, LV_EVENT_ALL,
		(void*)Btn_No, 0);
	...
	lv_obj_align(no_btn_, LV_ALIGN_BOTTOM_MID, size_w(80), -size_h(10));
...
void PageSysSet::OpenFormatTipWin()
{
	...
	format_tip_win_ = lv_create_page(lv_scr_act(), 260, 140, lv_palette_darken(LV_PALETTE_GREY, 2), 20, 2,
		lv_font_all, lv_color_white(), 0);
	lv_obj_align(format_tip_win_, LV_ALIGN_CENTER, 0, 0);

	lv_obj_t* label = lv_create_label(format_tip_win_, 260, GetParsedString("Please format the memory card"), LV_TEXT_ALIGN_CENTER, 0);
	lv_obj_align(label, LV_ALIGN_TOP_MID, 0, size_h(10));

	lv_obj_t* yes_btn = lv_create_btn(format_tip_win_, size_w(120), size_h(42),
	...
	lv_obj_align(yes_btn, LV_ALIGN_BOTTOM_LEFT, size_w(5), -size_h(20));

	lv_obj_t* btn_label = lv_label_create(yes_btn);
	lv_label_set_text(btn_label, GetParsedString("Confirm"));
	lv_obj_center(btn_label);

	lv_obj_t* no_btn = lv_create_btn(format_tip_win_, size_w(120), size_h(42),
	...
	lv_obj_align(no_btn, LV_ALIGN_BOTTOM_RIGHT, -size_w(5), -size_h(20));
...
void PageSysSet::OpenFormatProgressPage()
{
	...
	int page_width = size_w(200);
	format_progress_page_ = lv_create_page(page, page_width, size_h(110),
...
void PageSysSet::OpenVoiceControlPage()
{
	...
	GlobalPage::Instance()->page_main()->SetListStyle(list, subpage_width_, 46);
...
void PageSysSet::OpenEntryPage()
{
	...
	if (cfg_value.int_value == SimpChinese) {
		...
		lv_obj_set_size(entry_page_[0], screen_width, size_h(140));
		...
		for (int i = 0; i < 12; i++) {
			entry_label[i] = lv_create_label(entry_page_[0], size_w(80), entry[i].c_str(),
...
void PageSysSet::OpenUpgraderProgressPage()
{
	...
	int page_width = size_w(220);
	lv_obj_t* upgrade_progress_page_ = lv_create_page(lv_scr_act(), page_width, size_h(100),
		lv_color_make(38, 38, 38), 0, 2, lv_font_all, lv_color_white(), 0);
	...
	lv_obj_set_size(upgrade_bar_, page_width, size_h(35));
...
void PageSysSet::OpenWifiSetPage()
{
	...
	lv_obj_align(qr_img, LV_ALIGN_TOP_LEFT, size_w(6), size_h(22));
  ...
  if(!GlobalPage::Instance()->page_main()->touch_plan){
	...
	lv_obj_align(yes_btn, LV_ALIGN_BOTTOM_MID, -size_w(80), -size_h(20));

	lv_obj_t* btn_label = lv_label_create(yes_btn);
	lv_label_set_text(btn_label, GetParsedString("Open"));
	lv_obj_center(btn_label);

	lv_obj_t* no_btn = lv_create_btn(page, 110, 40, lv_color_make(95, 93, 93), 0, WifiEvent, LV_EVENT_ALL,
		(void*)Btn_No, 0);
	// lv_obj_set_style_border_width(no_btn, 2, 0);
	// lv_obj_set_style_border_color(no_btn, lv_palette_lighten(LV_PALETTE_BLUE_GREY, 2), 0);
	lv_obj_set_ext_click_area(no_btn, 15);
	lv_obj_align(no_btn, LV_ALIGN_BOTTOM_MID, size_w(80), -size_h(20));
