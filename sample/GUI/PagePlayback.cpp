...
void PagePlayback::CreatePage()
{
	...
	lv_style_set_width(&style_main, size_w(160));
	lv_style_set_height(&style_main, size_h(3));
	...
	lv_style_set_pad_all(&style_knob, 5); /*Makes the knob larger*/
...
void PagePlayback::OpenSetPage()
{
    ...
    lv_obj_t* top_page = lv_create_page(video_set_page_, screen_width, size_h(46),
	...
	for (int i = 0;i < PlaybackSubpage_Total;i++) {
		...
		lv_obj_t* label = lv_create_label(set_btn_[i], size_w(320), text[i].c_str(), LV_ALIGN_TOP_LEFT, 0);
...
void PagePlayback::OpenConfirmPage(int flag)
{
	...
	lv_obj_t* tip_win = lv_create_page(page, 320, 190, lv_palette_darken(LV_PALETTE_GREY, 2), 5, 2,
		lv_font_all, lv_color_white(), 0);
	lv_obj_align(tip_win, LV_ALIGN_CENTER, 0, size_h(23));

	std::string text[PlaybackSubpage_Total] = { GetParsedString("Delete current"), GetParsedString("Delete all"),
    GetParsedString("Lock current"), GetParsedString("Unlock current"),
    GetParsedString("Lock all"), GetParsedString("Unlock all") };
	lv_obj_t* label = lv_create_label(tip_win, 300, text[flag].c_str(), LV_TEXT_ALIGN_CENTER, 0);
	lv_obj_align(label, LV_ALIGN_TOP_MID, 0, size_h(40));

	lv_obj_t* yes_btn = lv_create_btn(tip_win, size_w(160), size_h(40),
	...
	lv_obj_t* no_btn = lv_create_btn(tip_win, size_w(160), size_h(40),
	...
	lv_obj_align(no_btn, LV_ALIGN_TOP_MID, size_w(0), size_h(140));
...
void PagePlayback::OpenSelectFilePage()
{
    ...
    lv_obj_t* top_page = lv_create_page(select_filetype_page_, screen_width, size_h(46),
        lv_color_make(68, 68, 68), 0, 0, lv_font_all, lv_color_white(), 0);
    lv_obj_align(top_page, LV_ALIGN_TOP_MID, 0, 0);
	...
	for (int i = 0;i < FileType_Total;i++) {
		...
		label = lv_create_label(filetype_img_[i], size_w(320), text[i].c_str(), LV_ALIGN_TOP_LEFT, 0);
