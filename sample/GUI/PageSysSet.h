...
public:
    ...
    lv_obj_t* gps_bar_graph[10];//信噪比柱状图
    lv_obj_t* gps_labe_graph[10];//信噪比文本
    lv_obj_t* gps_status_label;//GPS状态
    lv_obj_t* Altitude_label;  //海拔（米）
    lv_obj_t* orientation_label;//方位（度）
    lv_obj_t* longitude_label;   //经度
    lv_obj_t* latitude_label;    //纬度
    lv_obj_t* gps_speed_label;    //速度
    lv_obj_t* utc_label;          //时间
    lv_obj_t* visual_label;       //可视
    lv_obj_t* available_label;    //可用
private:
	int format_result_;
