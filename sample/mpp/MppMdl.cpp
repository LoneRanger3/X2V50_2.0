...
#define DISP_ROTATE_SET ROTATE_270
#define DISP_ROTATE_180 1

static int HOR_RES = 320;//416
static int VER_RES = 176; //240
static int HOR_UI_RES = 320; 
static int VER_UI_RES = 240; 
static int BTN_LIMIT = 32; //0
const int AD_VER_LIMIT =176;//240
...
void MppMdl::ShowLogo(PlayLogo logo)
{
	...
	if (logo == PlayLogo_Start) {
		...
		logo_path = "/usr/share/shut_down240x320.rgb";
	}
	else {
		return;
	}
...
int MppMdl::SetEncChnAttr(int channel, int stream, VENC_CHN_ATTR_S& enc_chn_attr)
{
	...
	if (video_codec_type == PT_H265) {
		...
		stH265Cbr.u32SrcFrmRate =kFrameRate; //frame_rate;//kFrameRate;
...
int MppMdl::ViConfig()
{
	...
	#if 0
	stViChnAttr.stDestSize.u32Width = 416;
	stViChnAttr.stDestSize.u32Height =VER_RES;
	CHECK_RET(XM_MPI_VI_SetChnAttr(vi_chn_sensor, &stViChnAttr), "XM_MPI_VI_SetChnAttr");
	CHECK_RET(XM_MPI_VI_EnableChn(vi_chn_sensor), "XM_MPI_VI_EnableChn");
	VI_CHN_CROP_S stViChnCrop;
	memset(&stViChnCrop, 0, sizeof(VI_CHN_CROP_S));
	stViChnCrop.stRect.s32X = 48;
	stViChnCrop.stRect.s32Y = 0;
	stViChnCrop.stRect.u32Width =HOR_RES;
	stViChnCrop.stRect.u32Height =VER_RES;
	stViChnCrop.bEnable =XM_TRUE;
    CHECK_RET(XM_MPI_VI_SetChnCrop(vi_chn_sensor, &stViChnCrop), "XM_MPI_VI_SetChnCrop");
	CHECK_RET(XM_MPI_VI_EnableChn(vi_chn_sensor), "XM_MPI_VI_EnableChn");
	#endif
